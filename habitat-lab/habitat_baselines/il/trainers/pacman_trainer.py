...
class PACMANTrainer(BaseILTrainer):
    ...
    def train(self) -> None:
        ...
        with habitat.Env(config.TASK_CONFIG) as env:
            ...
            train_loader = DataLoader(nav_dataset, batch_size=config.IL.NAV.batch_size)
                ...
                while epoch <= config.IL.NAV.max_epochs:
                    ...
                    for t, batch in enumerate(train_loader):
                        ...
                        planner_loss = planner_loss_fn(
                            planner_logprob,
                            planner_actions_out[:, : planner_action_lengths.max()].reshape(-1, 1),
                            planner_masks[:, : planner_action_lengths.max()].reshape(-1, 1),
                        )

                        controller_loss = controller_loss_fn(
                            controller_logprob,
                            controller_outs[:, : controller_action_lengths.max()].reshape(-1, 1),
                            controller_masks[:, : controller_action_lengths.max()].reshape(-1, 1),
                        )
                    ...
                    num_batches = math.ceil(len(nav_dataset) / config.IL.NAV.batch_size)
    ...
    def _eval_checkpoint(
        ...
        with habitat.Env(config.TASK_CONFIG) as env:
            ...
            for t, batch in enumerate(eval_loader):
                    ...
                    for j in ["pred", "fwd-only"]:
                        ...
                        if j == "pred":
                            ...
                            for step in range(planner_actions_in.size(0)):
                                ...
                                (planner_scores, planner_hidden,) = model.planner_step(
                                    question,
                                    planner_img_feats[step][(None,) * 2],  # unsqueezing twice
                                    planner_actions_in[step].view(1, 1),
                                    planner_hidden,
                                )

                        env.sim.set_agent_state(init_pos.position, init_pos.rotation)
                        init_dist_to_target = env.sim.geodesic_distance(init_pos.position, goal_pos)

                        if init_dist_to_target < 0 or init_dist_to_target == float("inf"):  # unreachable
                        ...
                        if j == "pred":
                            ...
                            if config.IL.NAV.max_controller_actions > 1:
                                controller_action_counter = (
                                    controller_action_counter % config.IL.NAV.max_controller_actions
                                )
                        ...
                        img = None
                        for episode_length in range(config.IL.NAV.max_episode_length):
                            if j == "pred":
                                if not first_step:
                                    if i == 30:  # saving results for 30-step walked back case
                                        imgs.append(img)
                                    img_feat = eval_loader.dataset.get_img_features(img, preprocess=True).view(
                                        1, 1, 4608
                                    )
                                else:
                                    img_feat = controller_img_feats.to(self.device).view(1, 1, 4608)

                                if not first_step or first_step_is_controller:
                                    # query controller to continue or not
                                    controller_action_in = torch.LongTensor(1, 1).fill_(action).to(self.device)
                                    ...
                                    if (
                                        controller_action == 1
                                        and controller_action_counter < config.IL.NAV.max_controller_actions - 1
                                    ):
                                ...
                                if planner_step:
                                    if not first_step:
                                        action_in = torch.LongTensor(1, 1).fill_(action + 1).to(self.device)
                                        (planner_scores, planner_hidden,) = model.planner_step(
                                            question,
                                            img_feat,
                                            action_in,
                            ...
                            episode_done = action == 3 or episode_length >= config.IL.NAV.max_episode_length

                            agent_pos = env.sim.get_agent_state().position

                            dists_to_target.append(env.sim.geodesic_distance(agent_pos, goal_pos))
                        ...
                        metrics_slug["d_D{}_{}".format(m, i)] = dists_to_target[0] - dists_to_target[-1]
                        metrics_slug["d_min{}_{}".format(m, i)] = np.array(dists_to_target).min()
                ...
                if t % config.LOG_INTERVAL == 0:
                    logger.info("Valid cases: {}; Invalid cases: {}".format((t + 1) * 8 - len(invalids), len(invalids)))
                    logger.info("EVAL: Avg metrics: {}".format(metrics.get_stat_string(mode=0)))
                    print("-----------------------------------------------------")
