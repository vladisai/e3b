...
class E2BTrainer(BaseRLTrainer):
    ...
    def _init_train(self):
        ...
        self.inverse_covs = torch.stack([torch.eye(hidden_size) * (1.0 / ridge) for i in range(num_envs)]).to(
            self.device
        )
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with torch.no_grad():
            ...
            torch.mul(
                self.outer_product_buffers,
                -1.0 / (1.0 + elliptical_bonus),
                out=self.outer_product_buffers,
            )
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        for i, done in enumerate(dones):
            if done:
                self.inverse_covs[i].copy_(torch.eye(self.inverse_covs.shape[1]) * (1.0 / self.config.RL.E2B.ridge))
