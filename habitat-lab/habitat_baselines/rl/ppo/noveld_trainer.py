...
class NovelDTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.predictor_net = PointNavResNetNet(
        ...
        self.target_net = PointNavResNetNet(
            observation_space=observation_space,
            action_space=None,
            hidden_size=self.config.RL.PPO.hidden_size,
            num_recurrent_layers=0,
        ...
        if self._is_distributed:
            self.predictor_net = DDP(
                self.predictor_net,
                device_ids=[self.device],
                output_device=self.device,
                find_unused_parameters=False,
            )
        self.predictor_net_optimizer = torch.optim.Adam(
            self.predictor_net.parameters(), lr=self.config.RL.E2B.encoder_lr
        )
    ...
    def _init_train(self):
        ...
        self.rnd_error_buffer = torch.zeros(num_envs, 1)
        self.prev_rnd_error_buffer = torch.zeros(num_envs, 1)
        self.indicator_buffer = torch.zeros(num_envs, 1)
        self.episodic_state_count_dicts = [defaultdict(lambda: 0) for _ in range(num_envs)]
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        rnd_error_diff = self.rnd_error_buffer - self.config.RL.NOVELD.alpha * self.prev_rnd_error_buffer
        bonus_rewards = self.indicator_buffer * torch.clamp(rnd_error_diff, min=0)
        bonus_rewards = torch.tensor(
            bonus_rewards.squeeze(),
            dtype=torch.float,
            device=self.current_episode_reward.device,
        )
