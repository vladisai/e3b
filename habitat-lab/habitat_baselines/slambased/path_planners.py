...
class DifferentiableStarPlanner(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.neights2channels.weight.data = torch.from_numpy(init_neights_to_channels(3))
        ...
        while not_done:
            ...
            if (ymin - 1 > 0) and (xmin - 1 > 0) and (ymax + 1 < self.height) and (xmax + 1 < self.width):
                ...
                self.g_map[:, :, ymin:ymax, xmin:xmax] = torch.min(
                    ...
                    (n2c + c_map[:, :, ymin:ymax, xmin:xmax]).min(dim=1, keepdim=True)[0],
                )
                ...
                self.open_list_map[:, :, ymin:ymax, xmin:xmax] = F.relu(
                    F.max_pool2d(
                        self.open_list_map[:, :, ymin - 1 : ymax + 1, xmin - 1 : xmax + 1],
                        3,
                        stride=1,
                        padding=0,
                    )
                ...
                self.g_map = torch.min(
                    self.g_map,
                    (self.neights2channels(F.pad(self.g_map, (1, 1, 1, 1), "replicate")) + c_map).min(
                        dim=1, keepdim=True
                    )[0],
                )
                self.close_list_map = torch.max(self.close_list_map, self.open_list_map)
                self.open_list_map = F.relu(
                    F.max_pool2d(self.open_list_map, 3, stride=1, padding=1) - self.close_list_map - self.obstacles
                )
            step += 1
    ...
    def reconstruct_path(self):
        ...
        count1 = 0
        while not done:
            node_coords = self.propagate_traversal(node_coords, self.close_list_map, self.g_map, self.coords)
            self.been_there[:, :, f2ind(node_coords, 0), f2ind(node_coords, 1)] = 1.0
