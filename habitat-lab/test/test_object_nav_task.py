...
def test_mp3d_object_nav_dataset():
    ...
    dataset.goals_by_category = {
        k: v for k, v in dataset.goals_by_category.items() if k in (ep.goals_key for ep in dataset.episodes)
    }
...
def test_object_nav_task():
    ...
    if not ObjectNavDatasetV1.check_config_paths_exist(config.DATASET):
        pytest.skip("Please download Matterport3D scene and ObjectNav Datasets to data folder.")
    ...
    with habitat.Env(config=config, dataset=dataset) as env:
        for _ in range(10):
            ...
            while not env.episode_over:
                ...
                habitat.logger.info(f"Action : " f"{action['action']}, " f"args: {action['action_args']}.")
                env.step(action)
