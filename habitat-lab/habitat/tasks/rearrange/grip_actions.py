    ...
    def _grasp(self):
        ...
        if len(scene_obj_pos) != 0:
            ...
            if to_target < self._config.GRASP_THRESH_DIST:
                self._sim.grasp_mgr.snap_to_obj(self._sim.scene_obj_ids[closest_obj_idx])
        ...
        if len(markers) > 0:
            ...
            closest_idx = np.argmin(np.linalg.norm(scene_obj_pos - ee_pos, ord=2, axis=-1))
...
class SuctionGraspAction(GripSimulatorTaskAction):
    ...
    def step(self, _, should_step=True, *args, **kwargs):
        ...
        robot_contacts = [c for c in contacts if coll_name_matches(c, robot_id)]

        # Contacted any objects?
        for scene_obj_id in self._sim.scene_obj_ids:
            has_robot_match = any(c for c in robot_contacts if coll_name_matches(c, scene_obj_id))
            if has_robot_match:
        ...
        for marker_name, marker in markers.items():
            has_match = any(
                ...
                if coll_name_matches(c, marker.ao_parent.object_id) and coll_link_name_matches(c, marker.link_id)
            )
            if has_match:
